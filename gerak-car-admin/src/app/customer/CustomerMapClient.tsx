  ...
  return (
    <div className="relative w-full h-full overflow-hidden flex flex-col bg-black">
      ...
      <div className="absolute top-6 left-6 z-30 pointer-events-auto">
         <button onClick={() => setShowMenu(true)} className="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-lg text-black hover:bg-zinc-200 transition-all">
      ...
      {showMenu && (
        <>
          ...
          <div className="absolute top-0 left-0 bottom-0 w-3/4 max-w-sm bg-white z-50 shadow-2xl flex flex-col transform transition-transform">
             ...
             <div className="flex-1 p-4 flex flex-col gap-2">
                <button className="flex items-center gap-4 p-4 hover:bg-zinc-100 rounded-xl transition-colors text-black font-bold">
             ...
             <div className="p-4 border-t border-zinc-200">
                <form action="/auth/signout" method="post">
                   <button type="submit" className="w-full bg-red-50 text-red-500 hover:bg-red-100 font-bold py-4 px-6 rounded-xl transition-all flex items-center justify-center gap-3">
      ...
      {!activePinMode && rideStatus === 'idle' && (
        <div className="absolute right-4 bottom-[400px] z-10 flex flex-col gap-3 pointer-events-none">
           <button onClick={() => {}} className="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-lg border border-zinc-200 pointer-events-auto text-black hover:bg-zinc-100 transition-colors">
      ...
      <div className="absolute bottom-0 left-0 right-0 z-20 w-full pointer-events-none flex flex-col items-center">
         <div className="w-full max-w-lg bg-white shadow-[0_-20px_40px_rgba(0,0,0,0.2)] pointer-events-auto flex flex-col p-6 pb-8 transition-transform duration-300 rounded-t-[32px] border-t border-zinc-200">
            ...
            {rideStatus !== 'idle' ? (
              ...
              <div className="flex flex-col items-center">
                 {rideStatus === 'pending' && (
                    <>
                       ...
                       <h3 className="text-black font-extrabold text-xl mb-1">Looking for drivers...</h3>
                       <p className="text-zinc-500 text-sm mb-6">Contacting nearby drivers</p>
                       <button onClick={cancelRide} className="w-full bg-zinc-100 hover:bg-zinc-200 text-black font-bold py-4 px-6 rounded-2xl transition-all">
                          Cancel Request
                       </button>
                    </>
                 )}
                 {rideStatus === 'accepted' && (
                    <>
                       ...
                       <h3 className="text-black font-extrabold text-xl mb-1">Driver is on the way</h3>
                       <p className="text-zinc-500 text-sm mb-6">Your driver accepted and is heading to you.</p>
                       <div className="w-full bg-zinc-100 rounded-2xl p-4 mb-4 flex items-center gap-4">
                          <div className="w-12 h-12 bg-zinc-300 rounded-full flex items-center justify-center text-xl">👨‍✈️</div>
                          <div>
                            <p className="text-black font-bold text-sm">Gerak Car Driver</p>
                            <p className="text-zinc-500 text-xs">Arriving shortly</p>
                          </div>
                       </div>
                    </>
                 )}
                 {rideStatus === 'arrived' && (
                    <>
                       ...
                       <p className="text-zinc-500 text-sm mb-6">Please meet your driver at the pickup point.</p>
                    </>
                 )}
                 {rideStatus === 'in_progress' && (
                    <>
                       ...
                       <h3 className="text-black font-extrabold text-xl mb-1">You are in transit</h3>
                       <p className="text-zinc-500 text-sm mb-6">Heading to your destination.</p>
                    </>
                 )}
                 {rideStatus === 'completed' && (
                    <>
                       ...
                       <h3 className="text-black font-extrabold text-2xl mb-1">You have arrived</h3>
                       <p className="text-[#00B14F] font-bold text-lg mb-6">RM 10.00</p>
                    </>
                 )}
              </div>
            ) : (
              /* REQUEST RIDE SHEET */
              <>
                <h2 className="text-black font-extrabold text-2xl mb-6">Where to?</h2>
                
                <div className="relative bg-zinc-100 border border-zinc-200 rounded-3xl p-4 mb-4">
                  {/* Vertical connecting line */}
                  <div className="absolute left-7 top-[34px] bottom-[34px] w-0.5 bg-zinc-300"></div>
                  ...
                  <div className="flex items-center gap-4 mb-4">
                     ...
                     <div 
                        ...
                        className={`flex-1 bg-transparent text-sm font-bold cursor-pointer truncate ${activePinMode === 'pickup' ? 'text-[#00B14F]' : 'text-zinc-700'}`}
                     >
                  ...
                  {/* Divider */}
                  <div className="h-px bg-zinc-200 ml-10 mb-4"></div>

                  {/* Dropoff Line */}
                  <div className="flex items-center gap-4">
                     <div className="w-6 h-6 flex items-center justify-center relative z-10 bg-zinc-100">
                     ...
                     <form onSubmit={handleSearch} className="flex-1 flex gap-2">
                       <input 
                          ...
                          className="w-full bg-transparent text-sm font-bold text-black placeholder-zinc-400 focus:outline-none"
                       />
                ...
                {searchResults.length > 0 && (
                  <div className="bg-white border border-zinc-200 rounded-2xl mb-4 max-h-40 overflow-y-auto z-10 shadow-lg">
                    {searchResults.map((res) => (
                      <button 
                        ...
                        className="w-full text-left px-4 py-3 border-b border-zinc-100 hover:bg-zinc-50 transition-colors flex items-center gap-3"
                      >
                        <span className="text-zinc-400">📍</span>
                        <div className="truncate">
                          <p className="text-black text-sm font-bold truncate">{res.display_name.split(',')[0]}</p>
                ...
                {isRouteSelected && (
                   <div className="flex flex-col gap-3 mb-6 animate-in slide-in-from-bottom-4 duration-300">
                      <div 
                         ...
                         className={`p-4 rounded-2xl flex items-center justify-between cursor-pointer border-2 transition-all ${selectedCar === 'economy' ? 'border-[#00B14F] bg-[#00B14F]/5' : 'border-zinc-200 hover:border-zinc-300'}`}
                      >
                         <div className="flex items-center gap-4">
                            <span className="text-3xl">🚗</span>
                            <div>
                               <h4 className="text-black font-extrabold text-base">GerakCar Economy</h4>
                               <p className="text-zinc-500 text-xs font-medium">Affordable everyday rides</p>
                            </div>
                         </div>
                         <div className="text-right">
                            <h4 className="text-black font-extrabold text-lg">RM 10.00</h4>
                            <p className="text-zinc-500 text-xs font-medium">3 mins away</p>
                         </div>
                      </div>

                      <div 
                         onClick={() => setSelectedCar('premium')}
                         className={`p-4 rounded-2xl flex items-center justify-between cursor-pointer border-2 transition-all ${selectedCar === 'premium' ? 'border-[#00B14F] bg-[#00B14F]/5' : 'border-zinc-200 hover:border-zinc-300'}`}
                      >
                         <div className="flex items-center gap-4">
                            <span className="text-3xl">🚙</span>
                            <div>
                               <h4 className="text-black font-extrabold text-base">GerakCar Premium</h4>
                               <p className="text-zinc-500 text-xs font-medium">More space and comfort</p>
                            </div>
                         </div>
                         <div className="text-right">
                            <h4 className="text-black font-extrabold text-lg">RM 15.00</h4>
                            <p className="text-zinc-500 text-xs font-medium">5 mins away</p>
                         </div>
                      </div>
                   </div>
                )}
