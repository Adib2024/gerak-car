...
export function ThemeToggle({ variant = 'pill' }: ThemeToggleProps) {
  ...
  if (!mounted) {
    if (variant === 'icon') {
      return <div className="w-10 h-10 bg-zinc-200 dark:bg-zinc-800 animate-pulse rounded-full"></div>
    }
  ...
  if (variant === 'icon') {
    return (
      <button
        ...
        className="w-10 h-10 flex items-center justify-center rounded-full bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors shadow-inner border border-zinc-200 dark:border-white/10"
        title="Toggle Theme"
      >
        {resolvedTheme === 'dark' ? (
